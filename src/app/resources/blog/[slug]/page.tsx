...
import {
  ArrowLeft, Clock, User, ArrowRight,
  CheckCircle2, AlertTriangle, Lightbulb, Info, BookOpen,
} from "lucide-react";
import { blogPosts, getBlogPost } from "@/src/lib/resources/blog-data";
    ...
    case "code":
      return (
        <div className="my-5 overflow-x-auto rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          <pre className="text-sm leading-relaxed text-emerald-300"><code>{block.code}</code></pre>
        </div>
      );
    case "divider":
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = blogPosts.filter((b) => post.relatedSlugs.includes(b.slug));
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex-1">
        {/* Hero */}
        <div className={`bg-gradient-to-br ${post.coverGradient} px-4 pb-12 pt-12 sm:px-6`}>
          <div className="mx-auto max-w-3xl">
            <span className={`inline-block rounded-full px-3 py-1 text-[11px] font-bold ${post.categoryColor}`}>
              {post.category}
            </span>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4 text-[12px] text-white/60">
              ...
              <span className="flex items-center gap-1.5"><User className="h-3.5 w-3.5" />{post.author} · {post.authorRole}</span>
        ...
        {related.length > 0 && (
          <section className="border-t border-slate-100 bg-surface px-4 py-10 sm:px-6">
            <div className="mx-auto max-w-3xl">
              <p className="mb-5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Related Articles</p>
              <div className="grid gap-4 sm:grid-cols-2">
                {related.map((r) => (
                  <Link key={r.slug} href={`/resources/blog/${r.slug}`}
                    ...
                    <div className="flex-1">
                      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold ${r.categoryColor}`}>{r.category}</span>
